    ...
    def get(self, request):
        try:
            ...
            if user.is_system_admin():
                ...
                queryset = AccessLog.objects.filter(user_id__in=user_ids)
            else:
            ...
            for item in cur_data:
                cur_ids.append(item)
            # read log detail
            page_data = AccessLog.objects.filter(id__in=cur_ids).order_by('-id')
            data = []
            for item in page_data:
                ...
                data.append({
                    ...
                    "user_id": user_id,
                    "username": user_name,
                    "action_time": format(item.start_time, 'Y-m-d H:i:s'),
                    "client_ip": item.client_ip,
                    "method": item.method,
                    "url": item.url,
                    "reply_code": item.status_code,
